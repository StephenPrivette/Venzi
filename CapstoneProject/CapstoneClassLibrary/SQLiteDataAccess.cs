...
namespace CapstoneClassLibrary
{
    public class SQLiteDataAccess
    {
        ...
        public void updateUserInfo(string username, string password, string newPassword)
        {
            using (SQLiteConnection cnn = new SQLiteConnection(loadConnectionString()))
            {
                cnn.Open();
                SQLiteCommand command = cnn.CreateCommand();
                command.CommandText = "update users set userPass = @param4 WHERE (userName = @param1) AND (userPass = @param2)";
                command.Parameters.Add(new SQLiteParameter("@param1", username));
                command.Parameters.Add(new SQLiteParameter("@param2", password));
                // command.Parameters.Add(new SQLiteParameter("@param3", newUsername));
                command.Parameters.Add(new SQLiteParameter("@param4", newPassword));
        ...
        public void deleteItineraryEvent(string username, string eventID)
        {
            // Deletes an itinerary item from the user's itinerary
            using (SQLiteConnection cnn = new SQLiteConnection(loadConnectionString()))
            {
                cnn.Open();
                SQLiteCommand command = cnn.CreateCommand();
                command.CommandText = "delete from " + username + "Itinerary WHERE (eventID) = (" + eventID + ")";
                command.ExecuteNonQuery();
                cnn.Close();
            }
        }
